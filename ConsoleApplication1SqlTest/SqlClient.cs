...
namespace ConsoleApplication1SqlTest
{
    class SqlClient
    {
        ...
        public SqlClient(string ip, string port, string user_id, string password, string database, string Pooling, string MinPoolSize, string MaxPoolSize, string ConnectionLifetime)
        {
            ...
            pgCSB.ConnectionTimeout = 5;
            pgCSB.Unicode = true;
            pgSqlConnection = new PgSqlConnection(pgCSB.ConnectionString);
            pgSqlConnection.ConnectionLost += new Devart.Common.ConnectionLostEventHandler(pgSqlConnection_ConnectionLost);

        }

        void pgSqlConnection_ConnectionLost(object sender, Devart.Common.ConnectionLostEventArgs e)
        {
            Console.WriteLine(DateTime.Now.ToString("F")+"ConnectLost");
        }
        public bool connect()
        {
            try
            ...
            catch (PgSqlException ex)
            {
                ...
                Console.WriteLine(DateTime.Now.ToString("G") + "-Connect exception occurs: {0}", ex.Error);
